  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await axios.post(
        "https://krishi-sakhi-backend-6.onrender.com/api/auth/login",
    ...
    <div style={S.wrap}>
      ...
      <form onSubmit={handleSubmit} style={S.form}>
        ...
        <div style={S.field}>
          ...
          <input type="password" required value={password} onChange={e => setPassword(e.target.value)} placeholder="••••••••" style={S.input} onFocus={e => e.target.style.borderColor="rgba(76,175,101,0.6)"} onBlur={e => e.target.style.borderColor="rgba(196,127,26,0.25)"} />
        </div>
...
const S = {
  ...
  btn:   { padding:"13px 0", borderRadius:10, border:"none", background:"linear-gradient(135deg,#2e6b3e,#4caf65)", color:"#fff", fontSize:15, fontWeight:700, fontFamily:"'DM Sans',sans-serif", marginTop:4, cursor:"pointer" },
};